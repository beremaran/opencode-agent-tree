...
const main = async () => {
    ...
    try {
        ...
        child.on("exit", (code, signal) => {
            if (code !== null && code !== 0) earlyExit = {message: `exit code ${code}${signal ? ` (${signal})` : ""}`};
        });
        ...
        for (const name of BUILTIN_SUBAGENTS) {
            ...
            if (modelStr(effective.model) !== SUBAGENT_MODEL) {
                fail(`agent "${name}" has plugin-injected model ${SUBAGENT_MODEL}`, `got ${modelStr(effective.model)}`);
            } else {
