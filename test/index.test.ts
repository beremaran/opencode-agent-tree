...
test("orchestrator permissions merge with a clobber warning", async () => {
    ...
    assert.match(overwrite[0].body.message, /Overwriting existing permission for tool "bash" on agent "Manager"/);
});
...
test("custom orchestrator is routed to primary mode with default blocked tools", async () => {
    const {config, logs} = await apply({subagentModel: "provider/model", orchestratorAgent: "lead"}, {agent: {}});
...
test("a pre-existing primary orchestrator mode does not warn about conversion", async () => {
    const {logs} = await apply({subagentModel: "provider/model"}, {agent: {Manager: {mode: "primary"}}});
...
test("blocking a directive tool warns but still applies the deny", async () => {
    const {config, logs} = await apply({subagentModel: "provider/model", blockedTools: ["task"]}, {agent: {}});
...
test("phantom agent names are created and logged with a warning", async () => {
    const {config, logs} = await apply({subagentModel: "provider/model", agents: ["typoAgent"]}, {agent: {}});
...
test("agent names colliding with Object.prototype keys are handled safely", async () => {
    const {config} = await apply({subagentModel: "provider/model", agents: ["toString"], agentModels: {}}, {agent: {}});
...
test("orchestratorModel null is treated as unset", async () => {
    const {config, logs} = await apply({subagentModel: "provider/model", orchestratorModel: null}, {agent: {}});
...
test("orchestrator agent is never routed even when listed in agentModels", async () => {
    ...
    const {config, logs} = await apply({subagentModel: "provider/model", agentModels: {Manager: "x/y"}}, {agent: {}});
...
test("blocked directive tools are joined into a single warning", async () => {
    const {config, logs} = await apply({subagentModel: "provider/model", blockedTools: ["task", "read"]}, {agent: {}});
...
test("summary log records routedAgents, blockedTools, and defaultAgent in extra", async () => {
    const {logs} = await apply({subagentModel: "provider/model"}, {default_agent: "coordinator", agent: {}});
...
test("restrictTask defaults to false and leaves an existing task rule untouched", async () => {
    for (const options of [{subagentModel: "provider/model"}, {subagentModel: "provider/model", restrictTask: false}]) {
...
test("a non-string orchestrator prompt logs a distinct plugin-bug error without throwing", async () => {
    const {logs} = await apply({subagentModel: "provider/model"}, {agent: {Manager: {mode: "primary", prompt: 42}}});
...
test("agents: ['general'] routes only general with no omission or phantom warnings", async () => {
    const {config, logs} = await apply({subagentModel: "provider/model", agents: ["general"]}, {agent: {}});
...
test("agents entries that are empty strings or non-strings are rejected", async () => {
    for (const agents of [[""], [42], ["general", ""]]) {
        const {input, logs} = createInput();
        await assert.rejects(() => OrchestratorPlugin(input, {subagentModel: "provider/model", agents}), /agents/);
        assert.equal(errorLogs(logs).length, 1);
    }
});
...
test("blocking a non-directive custom tool applies deny with no directive warning", async () => {
    const {config, logs} = await apply({subagentModel: "provider/model", blockedTools: ["custom_tool"]}, {agent: {}});
...
const creationLogs = (logs: LogEntry[]) =>
    logs.filter((entry) => entry.body.level === "info" && entry.body.message.startsWith("Creating orchestrator agent"));
...
test("orchestratorDepth 2 creates a Manager -> Manager-2 chain with structural task pinning", async () => {
    ...
    assert.match(manager2Prompt, /# Orchestrator Mode \(level 2\/2, enforced by @beremaran\/opencode-agent-tree\)/);
...
test("orchestratorDepth 3 creates a three-level chain; only the final level delegates to workers", async () => {
    const {config, logs} = await apply({subagentModel: "fallback/model", orchestratorDepth: 3}, {agent: {}});
...
test("chain final levels always get a task rule so the task tool is never stripped", async () => {
    const {config, hooks, logs} = await apply({subagentModel: "fallback/model", orchestratorDepth: 3}, {agent: {}});
...
test("re-running the config hook with orchestratorDepth 2 is idempotent", async () => {
    const {config, hooks, logs} = await apply({subagentModel: "fallback/model", orchestratorDepth: 2}, {agent: {}});
...
test("no subagent_depth warning when subagent_depth is sufficient", async () => {
    const {logs} = await apply({subagentModel: "provider/model", orchestratorDepth: 2}, {subagent_depth: 3, agent: {}});

    assert.equal(warnMatching(logs, /subagent_depth/).length, 0);
});

test("warns when orchestratorDepth 3 exceeds subagent_depth 2", async () => {
    const {logs} = await apply({subagentModel: "provider/model", orchestratorDepth: 3}, {subagent_depth: 2, agent: {}});
...
test("explicit subagent_depth 0 warns even for orchestratorDepth 1", async () => {
    const {logs} = await apply({subagentModel: "provider/model", orchestratorDepth: 1}, {subagent_depth: 0, agent: {}});
...
test("the hook does not intercept non-task tools", async () => {
    ...
    for (const tool of ["read", "bash", "edit", "todowrite"]) {
        await assert.doesNotReject(() => hook({tool, sessionID: "s1", callID: "c1"}, {args: {prompt: "anything"}}));
    }
    assert.equal(warnMatching(logs, /Delegation rejected/).length, 0);
});
...
const applyV2 = async (options: Record<string, unknown>, initial: V2TestAgent[] = []) => {
    ...
    let runtimeHook: ((event: {tool: string; agent: string; input: unknown}) => Promise<void> | void) | undefined;
