...
5. **Watch the publish workflow.** It verifies the tag matches `package.json`
   and that `CHANGELOG.md` contains the version, runs `bun run check`,
...
- A local `bun publish` is not the supported flow and will not produce npm
  provenance. `prepublishOnly` runs `bun run check` if you ever do, but prefer
  the tag flow.
